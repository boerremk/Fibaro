...
local zonnescherm = 807;
local User = json.decode(fibaro:getGlobalValue("User"))
  ...
  if rain and buienradar == "0" then
    ...
    if rain then
      ...
      if currentTime >= startTime and currentTime <= stopTime then
        ...
        fibaro:startScene(sendMessageID,{{true, {User["phoneid"]}},{false},{false},{true, "100"},{false},"Buienradar",msg})
        log(msg)
        if tonumber(fibaro:getValue(zonnescherm, "value")) > 0 then
          fibaro:call(zonnescherm, "setValue", "0")
          log("Zonnescherm ("..zonnescherm..") gesloten")
        end
      end
      fibaro:call(selfId, "setProperty", "ui.lblOverview.value", "Expected at "..raintime);
    end
